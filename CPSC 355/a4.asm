...
equal:     stp      fp,lr,[sp,alloc]!
           ...
           ldr      w23,[x20,size_s + height_s] //w23 = b1->size.height 
           ldr      w24,[x21,size_s + height_s] //w24 = b2->size.height
           cmp      w23,w24                     //if w23 == w24
           b.ne     return
          
           mov      w22,TRUE                    //result = true
...
//end of equal subroutine
           Alloc = -(16 + box_size*2) & -16     // allocating size in memory for box1 and box2 size
           Dealloc = - Alloc           //deallocating memory
           box1_s = 16                 //box1 offset
           box2_s = box1_s + box_size   //box2 offset
           .global main

main:      stp      fp,lr,[sp,Alloc]!
           mov      fp,sp

           add      x19,fp,box1_s            //calculating the base address for box1 in x19
           add      x20,fp,box2_s            //calulating the base address for box2 in x20


           mov      x0,x19                    //first = newBox()
           bl       newBox

           mov      x0,x20                   //second = newBox()
           bl       newBox
          
           adrp     x0,fmt4                //printf("Initial box values:\n")
           add      w0,w0,:lo12:fmt4 
           bl       printf

           adrp     x0,fmt2               //printBox("first",&first)
           add      x0,x0,:lo12:fmt2
           mov      x1,x19
           bl       printBox

           adrp     x0,fmt3               //printBox("second",&second)
           add      x0,x0,:lo12:fmt3
           mov      x1,x20
           bl       printBox

           mov      x0,x19             //equal(&first,&second)
           mov      x1,x20
           bl       equal
           mov      x23,x0             //w23 = equal result
           
           cmp      x23,TRUE          //if w23 == true
           b.ne     print_a
          
           mov      x0,x19           //move(&first,-5,7)
           mov      w1,-5
           mov      w2,7
           bl       move
           
           mov      x0,x20           //expand(&second,3)
           mov      w1,3
           bl       expand

print_a:   adrp     x0,fmt5          //printf("\nChanged box values:\n")
           add      w0,w0,:lo12:fmt5
           bl       printf
